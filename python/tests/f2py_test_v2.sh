...
test_molecule() {
    ...
    for i in "${!PYTHON_ARRAY[@]}"; do
        ...
        # Calculate percent difference in scientific notation
        if (( $(echo "($FORTRAN_VAL) != 0" | bc -l) )); then
            PERCENT_DIFF=$(echo "scale=10; ($DIFF/($FORTRAN_VAL))*100" | bc)
            
            # Convert to scientific notation using awk
            SCIENTIFIC_NOTATION=$(echo "$PERCENT_DIFF" | awk '{printf "%.2e", $1}')
        else
            # If Fortran value is zero, we can't calculate percentage
            # Instead, report the absolute difference in scientific notation
            SCIENTIFIC_NOTATION=$(echo "$DIFF" | awk '{printf "%.2e", $1}')
            SCIENTIFIC_NOTATION="${SCIENTIFIC_NOTATION} (absolute)"
        fi
    ...
    # Convert total difference to scientific notation
    TOTAL_DIFF_SCIENTIFIC=$(echo "$TOTAL_DIFF" | awk '{printf "%.2e", $1}')
    
    # Print result for this molecule
    if [ "$ALL_MATCH" = true ]; then
        echo -e "${GREEN}✅ SUCCESS: All values match within tolerance${NC}"
        echo "Total cumulative difference: $TOTAL_DIFF_SCIENTIFIC"
        return 0
    else
