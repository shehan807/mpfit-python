...
def dkaisq(p0: np.ndarray, state: ConstrainedMPFITState, xp=np) -> np.ndarray:
    ...
    # dparam1[atom_i * n_sites + site_s] is the gradient w.r.t. allcharge[s, i]
    # d/d(allcharge[s,a]) of conchg*(sum_mol_q - Q)^2 = conchg*2*(sum_mol_q - Q)
    # only for atoms a in that molecule (since qstore[a] = sum_s allcharge[s,a])
    if state.molecule_charges and state.atom_counts:
        offset = 0
        for n_i, q_target in zip(state.atom_counts, state.molecule_charges):
            mol_q = np.sum(state.qstore[offset:offset + n_i])
...
def optimize_constrained(
    ...
    Uses SLSQP with hard per-molecule charge equality constraints when
    molecule_charges are specified, otherwise falls back to L-BFGS-B with
    a soft penalty.
    """
    n_params = count_parameters(state)
    if p0_init is None:
    ...
    # Build per-molecule equality constraints for SLSQP
    constraints = []
    if state.molecule_charges and state.atom_counts:
        offset = 0
        for n_i, q_target in zip(state.atom_counts, state.molecule_charges):
            ...
            def mol_charge_constraint(p0, _offset=mol_offset, _n=mol_n, _target=mol_target):
                expandcharge(p0, state)
                return np.sum(state.qstore[_offset:_offset + _n]) - _target

            constraints.append({"type": "eq", "fun": mol_charge_constraint})
            offset += n_i

    if constraints:
        # Hard constraints: zero out soft penalty to avoid double-counting
        saved_conchg = state.conchg
        state.conchg = 0.0
        ...
        state.conchg = saved_conchg
    else:
        # No per-molecule constraints: use L-BFGS-B with soft penalty
        result = minimize(
            fun=lambda p: kaisq(p, state, xp=xp),
            x0=p0_init,
            jac=lambda p: dkaisq(p, state, xp=xp),
...
def fit_constrained_mpfit(
    ...
    Charge conservation is enforced per-molecule via penalty terms.
